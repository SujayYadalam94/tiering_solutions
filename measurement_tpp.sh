...
for workload_id in "${WORKLOAD_IDS[@]}"; do
    ...
    if ! measurement_workload_supports_system tpp && ! measurement_workload_supports_system arms; then
        echo "Skipping ${workload_id}: not supported by TPP"
        continue
    fi
