...
state->config.add_listeners(state->dram_bw, { vulcan::listeners::global::EWMA({0.3}) });
state->config.add_listeners(state->nvm_bw, { vulcan::listeners::global::EWMA({0.3}) });
...
auto scoring_fn = [](const vulcan::feature_store& fs, int64_t obj_id) -> double {
    ...
    double ewma_accesses = fs.get_ewma(state->accesses, obj_id, 0.5);
    
    // Get rolling average for stability
    double rolling_avg = fs.get_avg(state->accesses, obj_id);
    ...
    double dram_bw = fs.get_ewma(state->dram_bw, 0.3);
    double nvm_bw = fs.get_ewma(state->nvm_bw, 0.3);
