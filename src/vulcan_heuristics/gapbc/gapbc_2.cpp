...
auto scoring_fn = [](const vulcan::feature_store& fs, int64_t obj_id) -> double {
    double ewma = fs.get_ewma(state->accesses, obj_id, 0.72);
    double p60 = fs.get_percentile(state->accesses, 0.6);
    double p80 = fs.get_percentile(state->accesses, 0.8);
    double dram_bw = fs.get_ewma(state->dram_bw, 0.25);
    double nvm_bw = fs.get_ewma(state->nvm_bw, 0.25);
