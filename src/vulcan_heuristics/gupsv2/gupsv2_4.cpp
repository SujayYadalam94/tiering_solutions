...
auto scoring_fn = [](const vulcan::feature_store& fs, int64_t obj_id) -> double {
    ...
    double fast_ewma = fs.get_ewma(state->accesses, obj_id, 0.85);
    // Slow EWMA for stable baseline
    double slow_ewma = fs.get_ewma(state->accesses, obj_id, 0.25);
    
    // Use p50 and p90 to define hot range
    double p50 = fs.get_percentile(state->accesses, 0.5); // p50: cold/hot boundary
    double p90 = fs.get_percentile(state->accesses, 0.9); // p90: clearly hot threshold
