...
auto scoring_fn = [](const vulcan::feature_store& fs, int64_t obj_id) -> double {
    ...
    double fast_ewma = fs.get_ewma(state->accesses, obj_id, 0.8);
    // Slower EWMA for baseline comparison
    double slow_ewma = fs.get_ewma(state->accesses, obj_id, 0.3);
    
    // Population median as normalization anchor
    double pop_median = fs.get_percentile(state->accesses, 0.5); // p50: normalization anchor
    double pop_p90 = fs.get_percentile(state->accesses, 0.9);    // p90: hot threshold
