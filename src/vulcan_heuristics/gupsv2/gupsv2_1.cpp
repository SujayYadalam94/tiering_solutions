...
auto scoring_fn = [](const vulcan::feature_store& fs, int64_t obj_id) -> double {
    double fast = fs.get_ewma(state->accesses, obj_id, 0.5);  // recent activity
    double slow = fs.get_ewma(state->accesses, obj_id, 0.1);  // long-term baseline
    
    double p50 = fs.get_percentile(state->accesses, 0.5);  // p50: population median anchor
