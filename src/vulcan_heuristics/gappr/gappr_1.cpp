...
auto scoring_fn = [](const vulcan::feature_store& fs, int64_t obj_id) -> double {
    double recent = fs.get_ewma(state->accesses, obj_id, 0.7);  // Recent activity
    double stable = fs.get_ewma(state->accesses, obj_id, 0.3);  // Sustained pattern
    ...
    double p50 = fs.get_percentile(state->accesses, 0.5);  // p50: median anchor
    double p75 = fs.get_percentile(state->accesses, 0.75); // p75: upper quartile threshold
