...
auto scoring_fn = [](const vulcan::feature_store& fs, int64_t obj_id) -> double {
    double fast = fs.get_ewma(state->accesses, obj_id, 0.7);  // Recent
    double slow = fs.get_ewma(state->accesses, obj_id, 0.3);  // Sustained
    
    // p75 normalization to emphasize truly hot pages
    double p75 = fs.get_percentile(state->accesses, 0.75);  // p75: upper quartile anchor
