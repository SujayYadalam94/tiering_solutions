...
state->config.add_listeners(state->dram_bw, { vulcan::listeners::global::EWMA({0.2}) });
state->config.add_listeners(state->nvm_bw, { vulcan::listeners::global::EWMA({0.2}) });
...
auto scoring_fn = [](const vulcan::feature_store& fs, int64_t obj_id) -> double {
    ...
    double ewma_slow = fs.get_ewma(state->accesses, obj_id, 0.3);
    double ewma_fast = fs.get_ewma(state->accesses, obj_id, 0.7);
    ...
    double p50 = fs.get_percentile(state->accesses, 0.5);
    double norm = (p50 > 0.0) ? p50 : 1.0;
    
    // Bandwidth ratio: how much faster is DRAM? Higher ratio = more benefit from promotion
    double dram = fs.get_ewma(state->dram_bw, 0.2);
    double nvm = fs.get_ewma(state->nvm_bw, 0.2);
