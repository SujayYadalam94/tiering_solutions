...
static offcore_write_l3_step_metrics snapshot_offcore_write_l3_step_metrics()
{
    ...
    const uint64_t step_any = (cumulative_any >= previous_offcore_rfo_any_response)
                                  ...
                                  : cumulative_any;
    const uint64_t step_miss_raw =
        (cumulative_miss >= previous_offcore_rfo_l3_miss) ? (cumulative_miss - previous_offcore_rfo_l3_miss)
                                                          : cumulative_miss;
...
static void write_offcore_write_l3_metrics_to_file()
{
    ...
    const double miss_rate = (demand_rfo_any > 0)
                                 ? (static_cast<double>(clamped_demand_rfo_l3_miss) / static_cast<double>(demand_rfo_any))
                                 : 0.0;
