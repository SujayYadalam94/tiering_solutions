# Tiering Solutions - TPP Branch

This branch contains the source of Linux v6.0 with changes related to TPP applies. It also has some subtle modifications to enable tiering across NUMA nodes. 

This branch also includes a patch to prioritize file page allocations on a remote NUMA node (node 1). This is useful to prevent TPP from trying to migrate file pages, TPP can focus on migrating only anonymous pages.
...
Additionally, you might want to reset kswapd stats. kswapd stops migrating if the number of migrations failures exceed a threshold.
Colloid's repo again has a useful tool to reset the stats periodically.
