...
sudo apt -y install \
    ...
    gfortran \
    cmake \
    libgflags-dev libsnappy-dev  zlib1g-dev libbz2-dev lz4 liblz4-1 liblz4-dev libzstd-dev \
...
sudo bash ubuntu-mainline-kernel.sh -i 6.18.1
...
sed -i -e 's/^#define FARMEM_NUMA 0$/#define FARMEM_NUMA 1/' -e 's/^#define LOCAL_NUMA 1$/#define LOCAL_NUMA 0/' ~/colloid/tpp/memeater/memeater.c
...
tmux new-session -d "make -j ; sudo make install ; sudo update-alternatives --install /usr/bin/gcc gcc /opt/gcc-15/bin/gcc 100 ; sudo update-alternatives --install /usr/bin/g++ g++ /opt/gcc-15/bin/g++ 100 ; sudo ln -s /usr/bin/gcc /usr/bin/gcc-15"
