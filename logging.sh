...
#run_program "/users/zimooo2/XSBench/openmp-threading/XSBench -t 16 -g 50000 -p 20000000" logging XSBench
...
for run in $(seq 1 "${RUNS}"); do
    ...
    for graph in "${graphs[@]}"; do
        for prog in "${gapbs_programs[@]}"; do
            ...
            run_program "/users/zimooo2/gapbs/$prog -n 40 -f /users/zimooo2/gapbs/benchmark/graphs/$graph" logging $prog-$graph ${run}
        done
    done
    ...
    for graph in "${graphs[@]}"; do
        for prog in "${gapbs_programs[@]}"; do
            echo "Run ${run}: Running GAPBS program: $prog on graph: $graph"
            run_program "/users/zimooo2/gapbs/$prog -n 20 -f /users/zimooo2/gapbs/benchmark/graphs/$graph" logging $prog-$graph ${run}
        done
    done
done
...
#run_program "/users/zimooo2/LULESH/build/lulesh2.0 -i 10 -s 400" logging lulesh2.0_s400
...
#run_program "/users/zimooo2/duckdb/build/release/benchmark/benchmark_runner benchmark/large/tpcds-sf100/.*benchmark --threads=16" logging DuckDB-TPCDS-sf100
