...
CGROUP_NAME="htmm"
###### update DIR!
DIR=/mnt/nvme/memtis/memtis-userspace
...
function func_memtis_setting() {
    sudo bash -c "echo 199 | tee /sys/kernel/mm/htmm/htmm_sample_period"
    sudo bash -c "echo 100007 | tee /sys/kernel/mm/htmm/htmm_inst_sample_period"
    sudo bash -c "echo 1 | tee /sys/kernel/mm/htmm/htmm_thres_hot"
    sudo bash -c "echo 2 | tee /sys/kernel/mm/htmm/htmm_split_period"
    sudo bash -c "echo 100000 | tee /sys/kernel/mm/htmm/htmm_adaptation_period"
    sudo bash -c "echo 2000000 | tee /sys/kernel/mm/htmm/htmm_cooling_period"
    sudo bash -c "echo 2 | tee /sys/kernel/mm/htmm/htmm_mode"
    sudo bash -c "echo 500 | tee /sys/kernel/mm/htmm/htmm_demotion_period_in_ms"
    sudo bash -c "echo 500 | tee /sys/kernel/mm/htmm/htmm_promotion_period_in_ms"
    sudo bash -c "echo 4 | tee /sys/kernel/mm/htmm/htmm_gamma"
    ###  cpu cap (per mille) for ksampled
    sudo bash -c "echo 30 | tee /sys/kernel/mm/htmm/ksampled_soft_cpu_quota"

    if [[ "x${CONFIG_NS}" == "xoff" ]]; then
	sudo bash -c "echo 1 | tee /sys/kernel/mm/htmm/htmm_thres_split"
    else
	sudo bash -c "echo 0 | tee /sys/kernel/mm/htmm/htmm_thres_split"
    fi

    if [[ "x${CONFIG_NW}" == "xoff" ]]; then
	sudo bash -c "echo 0 | tee /sys/kernel/mm/htmm/htmm_nowarm"
    else
	sudo bash -c "echo 1 | tee /sys/kernel/mm/htmm/htmm_nowarm"
    fi

    if [[ "x${CONFIG_CXL_MODE}" == "xon" ]]; then
	#${DIR}/scripts/set_uncore_freq.sh on
	sudo bash -c "echo "enabled" | tee /sys/kernel/mm/htmm/htmm_cxl_mode"
    else
	#${DIR}/scripts/set_uncore_freq.sh off
	sudo bash -c "echo "disabled" | tee /sys/kernel/mm/htmm/htmm_cxl_mode"
    fi
