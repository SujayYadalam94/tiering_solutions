...
mkdir -p times logs times/baseline

function run_workload {
    local run=$1
    local time_basename="${SIZE_MIB}MiB_run${run}"
    local time_dir="${SCRIPT_DIR}/times/baseline/${WORKLOAD_OUTPUT}"
    local log_dir="${SCRIPT_DIR}/logs/${WORKLOAD_OUTPUT}"
    local time_file="${time_dir}/${time_basename}.time"
    local log_output_path="${log_dir}/${time_basename}_baseline.log"
...
for workload_id in "${WORKLOAD_IDS[@]}"; do
    ...
    if ! measurement_workload_supports_system arms; then
        echo "Skipping ${workload_id}: not supported by baseline"
        continue
    fi
